...
class CKasperskyScanner(CAvScanner):
    ...
    def scan(self, path):
        ...
        # stderr=devnull because kavscanner writes socket info
        try:
...
class CAvgScanner(CAvScanner):
    ...
    def __init__(self, cfg_parser):
        ...
        # like this:Luhe.Fiha.A
        self.pattern2 = "\>{0,1}(.*) \s+[a-z]+\s+(.*)"
    ...
    def scan(self, path):
        ...
        matches1 = re.findall(self.pattern1, output,
                              re.IGNORECASE | re.MULTILINE)
        matches2 = re.findall(self.pattern2, output,
                              re.IGNORECASE | re.MULTILINE)
